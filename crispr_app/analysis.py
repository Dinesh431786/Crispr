...
with tab_sim:
    ...
    selected_gRNA = st.selectbox("gRNA", g_list, key="sel_gRNA")
    edit_label = st.selectbox("Edit type", list(EDIT_TYPES.keys()), key="edit_label")
    ...
    if st.session_state.protein_domains is not None:
        ...
        if not st.session_state.protein_domains.empty:
            ...
            fig = plot_protein_domains(before, st.session_state.protein_domains, cut_aa)
