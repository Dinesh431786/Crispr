...
# ---------- STEP 4: OFF-TARGETS & RANKING ----------
    ...
    with st.expander("⚙️ Advanced Off-target Settings", expanded=False):
        bg_seq = st.text_area("Background DNA (for off-target search): Paste any DNA region here (NOT gRNA).", key="bg_seq")
        max_mm = st.slider("Max mismatches", 0, 4, 2, key="max_mm")
    if st.button("🔬 Scan off-targets"):
        # Validate background sequence
        ok, _ = validate_sequence(bg_seq, allow_n=True)
        if not ok:
    ...
    if ot_df is not None and not ot_df.empty:
        ...
        st.subheader("Off-target Results")
        st.info("No off-targets found! Try pasting a DNA sequence (not a gRNA) in the background above.")
...
# ---------- STEP 5: AI EXPLAIN / GEMINI REPORT ----------
    ...
    # Compose a more informative context if results available
    context = ""
    if st.session_state.df_guides is not None and not st.session_state.df_guides.empty:
        context += "Top gRNAs:\n" + st.session_state.df_guides.head(10).to_csv(index=False)
    if st.session_state.offtargets is not None and not st.session_state.offtargets.empty:
        context += "\nOff-targets:\n" + st.session_state.offtargets.head(10).to_csv(index=False)
    full_prompt = context + "\n" + prompt
