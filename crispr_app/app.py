...
# ---------- STEP 2: gRNA DESIGN ----------
    ...
    st.markdown(f"""
        <div style="background-color:#f4fbfa; border-radius:8px; padding:12px;">
        <b>Design Summary:</b>  
...
# ---------- STEP 4: OFF-TARGETS & RANKING ----------
    ...
    st.info(
        "If you want to test off-targets, paste sequences **similar to your gRNAs** here. "
        "For demo: Paste a gRNA (or 1–4 mismatches) from Step 2."
    )
    df = st.session_state.df_guides
    if df is None or df.empty:
    ...
    if st.button("🔬 Scan off-targets"):
        ...
        if result_from_find is not None and not result_from_find.empty:
    ...
    if ot_df is not None and not ot_df.empty:
        ...
        st.download_button(
            label="⬇️ Download off-targets",
            data=ot_df.to_csv(index=False),
            file_name="offtargets.csv",
            mime="text/csv"
        )
    else:
        st.info("No off-targets found! Try pasting a gRNA sequence (or a variant with a few mismatches) in the background above.")
